...
抽象类总结规定</br>
1. 抽象类不能被实例化(初学者很容易犯的错)，如果被实例化，就会报错，编译无法通过。只有抽象类的非抽象子类可以创建对象。 即不能 new 对象 </br>
...
## 高效代码
  1.常量 & 变量</br> 
     1.1 直接赋值常量值，禁止声明新对象</br> 
     1.2 当成员变量值无需改变时，尽量定义为静态常量</br> 
     1.3 尽量使用基本数据类型，避免自动装箱和拆箱</br> 
     1.4 如果变量的初始值会被覆盖，就没有必要给变量赋初始值</br> 
     1.5 尽量使用函数内的基本类型临时变量</br> 
     1.6 尽量不要在循环体外定义变量</br> 
     1.7 不可变的静态变量，尽量使用非线程安全类</br> 
     1.8 不可变的成员变量，尽量使用非线程安全类</br> 
   2.对象&类
     2.1 禁止使用JSON转化对象</br> 
     2.2 尽量不使用反射赋值对象</br> 
     2.3 采用Lambda表达式替换内部匿名类</br> 
     2.4 尽量避免定义不必要的子类</br> 
     2.5 尽量指定类的final修饰符</br> 
   3.方法
     3.1 把跟类成员变量无关的方法声明成静态方法</br> 
     3.2 尽量使用基本数据类型作为方法参数类型，避免不必要的装箱、拆箱和空指针判断</br> 
     3.3 尽量使用基本数据类型作为返回值类型，避免不必要的装箱、拆箱和空指针判断</br> 
     3.4 协议方法参数值非空，避免不必要的空指针判断</br> 
## 常用库(github)
EventBus   线程组件间通信
1.控件按钮到fram之前的通信
...
自动按键打怪 Java控制台程序 自动识别文本判断开发 识别
